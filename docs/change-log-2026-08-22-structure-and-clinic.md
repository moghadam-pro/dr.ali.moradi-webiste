...
فایل عمومی `scripts/deploy-uploaded-release.sh` برای حالتی اضافه شد که کل Repository
ابتدا در یک پوشه موقت روی سرور Linux آپلود می‌شود. اسکریپت پیش از جایگزینی Build
می‌گیرد، از نسخه فعال Backup می‌سازد، فایل‌های `.env` و Git سرور را حفظ می‌کند،
PM2 یا systemd را Restart می‌کند و در صورت شکست Health Check نسخه قبلی را
بازمی‌گرداند. مسیرها و نام سرویس همچنان فقط هنگام اجرا وارد می‌شوند و در Git
ذخیره نشده‌اند.
