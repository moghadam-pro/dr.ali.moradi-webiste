...
## بررسی خارجی پس از انتقال — ۲۰۲۶-۰۹-۲۲

بررسی Read-only از اینترنت عمومی نشان می‌دهد وضعیت عملیاتی از آخرین ثبت این سند
جلوتر رفته است:

- `https://dralimoradi.com/` اکنون قالب جدید `dr-ali-moradi` و محتوای سه‌زبانه
  منتقل‌شده از Staging را ارائه می‌کند. صفحه اصلی و نمونه‌های Condition در انگلیسی،
  فارسی و عربی پاسخ موفق دارند و جهت فارسی/عربی RTL است.
- درخواست HTTP دامنه اصلی به HTTPS نهایی می‌شود؛ اجبار HTTPS در لایه عمومی فعال است.
- `https://legacy.dralimoradi.com/` به‌صورت مستقل پاسخ موفق می‌دهد و دیگر به
  Production ریدایرکت نمی‌شود.
- چند لینک ناوبری داخل Legacy همچنان URL مطلق `dralimoradi.com` دارند (برای نمونه
  مسیرهای Clinic/Research/Education/News/About). آرشیو بالا است، اما برای حفظ
  ناوبری تاریخی مستقل باید این URLهای درون‌محتوا با ابزار Serialization-safe به
  دامنه Legacy بازنویسی یا عمداً به‌عنوان لینک خروجی مستند شوند.
- فرم Contact عربی روی Production هنوز بومی‌سازی کامل نشده و برچسب‌ها، گزینه‌ها
  و دکمه Submit را به انگلیسی نشان می‌دهد.
- اجرای Duplicator، پاک‌شدن فایل‌های Installer/Archive، حذف دیتابیس خالی و وضعیت
  نهایی Backupها از دسترسی عمومی قابل اثبات نیست. تا زمان تأیید داخل DirectAdmin
  این موارد نباید انجام‌شده فرض شوند.

این بخش جایگزین چک‌لیست عملیاتی زیر نیست؛ فقط فاصله بین وضعیت زنده و آخرین Commit
ثبت‌شده در Repository را مستند می‌کند.
