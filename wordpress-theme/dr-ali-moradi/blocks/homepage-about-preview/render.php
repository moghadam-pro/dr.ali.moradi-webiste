<?php
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

$locale  = dam_current_locale();
$t       = dam_site_copy( $locale );
$about_url    = dam_localized_page_url( 'about', $locale );
$research_url = dam_localized_page_url( 'research', $locale );
$office_image = dam_media_url( 'office', DAM_THEME_URI . '/assets/img/about/office.jpg' );

$kicker          = dam_theme_mod( 'about_kicker', $locale );
$body            = dam_theme_mod( 'about_body', $locale );
$cta_label       = dam_theme_mod( 'about_cta_label', $locale );
$research_label  = dam_theme_mod( 'about_research_label', $locale );
?>
<section <?php echo get_block_wrapper_attributes( array( 'class' => 'about-preview section-space' ) ); ?>>
	<div class="section-shell about-grid">
		<div class="about-copy reveal">
			<p class="section-index light"><?php echo esc_html( $kicker ); ?></p>
			<p><?php echo esc_html( $body ); ?></p>
			<div class="about-actions">
				<a class="button button-light" href="<?php echo esc_url( $about_url ); ?>"><?php echo esc_html( $cta_label ); ?><?php echo dam_icon( 'arrow-right', 17 ); ?></a>
				<a class="text-link light" href="<?php echo esc_url( $research_url ); ?>"><?php echo esc_html( $research_label ); ?><?php echo dam_icon( 'file-text', 17 ); ?></a>
			</div>
		</div>
		<div class="about-media reveal">
			<div class="about-rings" aria-hidden="true"></div>
			<div class="pulse-line" aria-hidden="true"></div>
			<img class="fill-img" src="<?php echo esc_url( $office_image ); ?>" alt="">
		</div>
	</div>
</section>
